...
with st.sidebar:
    ...
    # EU member country names (Russian, as Metrika returns them)
    EU_COUNTRY_NAMES = {
    ...
    # Resolve EU country IDs from regionCountry dim (id + name together)
    def get_eu_region_ids():
        d = stat("ym:s:visits", dims="ym:s:regionCountry",
                 date1="365daysAgo", date2="today", limit=300)
        ids, names_matched, debug_rows = [], [], []
        if "data" in d:
            for row in d["data"]:
                dim = row["dimensions"][0]
                cid   = dim.get("id")
                cname = dim.get("name", "")
                debug_rows.append((cname, cid))
                if cname in EU_COUNTRY_NAMES:
                    names_matched.append(cname)
                    if cid is not None:
                        ids.append(str(cid))
        return ids, names_matched, debug_rows
    EU_REGION_IDS_LIST, _eu_names_matched, _eu_debug_rows = get_eu_region_ids()

    country_raw = stat("ym:s:visits", dims="ym:s:regionCountryName", limit=50)
    countries = ["Все страны", "🇪🇺 Европа (ЕС)"]
    ...
    filters = []
    if country == "🇪🇺 Европа (ЕС)":
        if EU_REGION_IDS_LIST:
            # IN(...) counts as ONE condition (avoids 20-condition limit)
            id_list = ",".join(f"'{i}'" for i in EU_REGION_IDS_LIST)
            filters.append(f"ym:s:regionCountry IN({id_list})")
        else:
            # Fallback by name, also via IN(...)
            name_list = ",".join(f"'{q(c)}'" for c in EU_COUNTRY_NAMES)
...
with st.expander("🔧 Отладка фильтров", expanded=False):
    ...
    if country == "🇪🇺 Европа (ЕС)":
        st.markdown("**Проверка названий стран ЕС (есть ли трафик):**")
        _eu_check = stat("ym:s:visits", dims="ym:s:regionCountryName",
                         date1=d1_main, date2=d2_main, limit=50)
        found_names = set()
        if "data" in _eu_check:
            for row in _eu_check["data"]:
                found_names.add(row["dimensions"][0].get("name",""))
        st.write(f"Найдено EU region IDs: {len(EU_REGION_IDS_LIST)}")
        st.write(f"Совпало по имени: {len(_eu_names_matched)} → {_eu_names_matched}")
        st.write(f"IDs: {EU_REGION_IDS_LIST}")
        st.markdown("**Что вернул regionCountry (name → id):**")
        for cname, cid in _eu_debug_rows[:40]:
            in_eu = "🟢" if cname in EU_COUNTRY_NAMES else "·"
            st.write(f"{in_eu} name={repr(cname)} id={repr(cid)}")
