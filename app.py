...
with st.sidebar:
    ...
    # Country presets (Russian names as Metrika returns them)
    EU_COUNTRIES = [
    ...
    EU1_COUNTRIES = EU_COUNTRIES[:14]   # Австрия … Ирландия
    EU2_COUNTRIES = EU_COUNTRIES[14:]   # Италия … Швеция
    EN_COUNTRIES = ["США", "Великобритания", "Австралия", "Новая Зеландия",
                    "Канада", "Ирландия"]

    # Build name→id map from regionCountry dim (id needed for reliable filtering)
    country_raw = stat("ym:s:visits", dims="ym:s:regionCountry", limit=300,
                       date1="365daysAgo", date2="today")
    all_countries = []
    country_id_map = {}   # name → numeric id
    if "data" in country_raw:
        for row in country_raw["data"]:
            ...
            cid = dim.get("id")
            if n:
                all_countries.append(n)
                if cid is not None:
                    country_id_map[n] = str(cid)
    ...
    if selected_countries:
        # Map selected names to numeric IDs (reliable for filtering)
        sel_ids = [country_id_map[c] for c in selected_countries if c in country_id_map]
        if sel_ids:
            # IN(...) counts as ONE condition — no limit problem
            id_list = ",".join(f"'{i}'" for i in sel_ids)
            filters.append(f"ym:s:regionCountry IN({id_list})")
        else:
            # Fallback: names via IN
            name_list = ",".join(f"'{q(c)}'" for c in selected_countries)
            filters.append(f"ym:s:regionCountryName IN({name_list})")
    if traffic_src_id:
...
with st.expander("🔧 Отладка фильтров", expanded=False):
    ...
    st.markdown("**Финальная строка фильтра (что уходит в API):**")
    st.code(build_filter(c_flt))
    _test = stat("ym:s:visits", date1=d1_main, date2=d2_main, flt=c_flt)
    if "error" in _test:
        st.error(f"Ошибка API: {_test['error']}")
    st.write("Визитов с текущим фильтром:", get_metric(_test, 0, 0))
    st.markdown("**Сырой ответ API (первые 500 символов):**")
    st.code(str(_test)[:500])

    st.markdown("**🧪 Автотест синтаксиса фильтра (Германия, тот же период):**")
    # Variant 1: single == by name
    t1 = stat("ym:s:visits", date1=d1_main, date2=d2_main,
              flt="ym:s:regionCountryName=='Германия'")
    st.write(f"1) ==name: {get_metric(t1,0,0)}  err={t1.get('error','-')}")
    # Variant 2: IN by name
    t2 = stat("ym:s:visits", date1=d1_main, date2=d2_main,
              flt="ym:s:regionCountryName IN('Германия','Франция')")
    st.write(f"2) IN names: {get_metric(t2,0,0)}  err={t2.get('error','-')}")
    # Variant 3: OR by name
    t3 = stat("ym:s:visits", date1=d1_main, date2=d2_main,
              flt="(ym:s:regionCountryName=='Германия' OR ym:s:regionCountryName=='Франция')")
    st.write(f"3) OR names: {get_metric(t3,0,0)}  err={t3.get('error','-')}")
    # Variant 4: no filter (baseline)
    t4 = stat("ym:s:visits", date1=d1_main, date2=d2_main)
    st.write(f"4) без фильтра: {get_metric(t4,0,0)}  err={t4.get('error','-')}")
    if selected_countries:
        st.write(f"Выбрано стран: {len(selected_countries)} → {selected_countries}")
