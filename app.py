...
with st.sidebar:
    ...
    preset = st.selectbox("🌍 Страны — пресет", [
        "Все страны", "🇪🇺 Европа (ЕС)", "🇬🇧 Англоязычные", "✏️ Выбрать вручную"
    ])

    if preset == "Все страны":
        selected_countries = []
    elif preset == "🇪🇺 Европа (ЕС)":
        selected_countries = [c for c in EU_COUNTRIES if c in all_countries]
    ...
    COUNTRY_CAP = 18
    country_overflow = len(selected_countries) > COUNTRY_CAP
    if country_overflow:
        st.warning(f"⚠️ Выбрано {len(selected_countries)} стран, "
                   f"Метрика поддерживает максимум ~{COUNTRY_CAP} в фильтре. "
                   f"Использую первые {COUNTRY_CAP} по трафику.")
        # Keep the ones with most traffic (all_countries is sorted by visits desc)
