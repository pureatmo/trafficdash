...
if "data" in src_m and src_m["data"]:
    ...
    with col2:
        if purchase_gid and "p" in top.columns:
            df_show = top[["src","v_A","p"]].copy()
            df_show["cr"] = df_show.apply(
                lambda r: f"{r['p']/r['v_A']*100:.2f}%" if r['v_A'] else "0%",
                axis=1)
            df_show.columns = ["Источник", f"Сеансы {label_main}",
                               "Покупки", "CR в покупку"]
