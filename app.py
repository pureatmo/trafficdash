...
def build_filter(flt):
    return NO_ROBOTS + (f" AND ({flt})" if flt else "")

@st.cache_data(ttl=600, show_spinner=False)
def _stat_cached(metrics, dims, date1, date2, f, limit):
    ...
    try:
        ...
        if r.status_code != 200:
            # Do NOT cache errors — raise so cache_data skips storing
...
def stat(metrics, dims=None, date1="today", date2="today", flt=None, limit=100):
    try:
        return _stat_cached(metrics, dims, date1, date2, build_filter(flt), limit)
    except Exception as e:
        return {"error": str(e)}
...
# Все пары работают в рамках одной сессии (без внешних редиректов)
PAIR_GOALS = [
    # (название перехода, ID составной цели-пары, знаменатель_keyword)
    ("Просмотр → Добавление в корзину", 562849814, "cannabis-seeds"),
    ("Просмотр корзины → Корзина",      562849950, "add_to_cart"),
    ("Корзина → Чекаут",                562850066, "basket"),
    ("Чекаут → Покупка",                562851178, "начало оформления"),
    ("Корзина → Покупка",               562851346, "basket"),
]

# ── sidebar ────────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("### 🌿 Traffic Dashboard")
    if st.button("🔄 Обновить данные (сбросить кэш)", use_container_width=True):
        st.cache_data.clear()
        st.rerun()
    st.markdown("---")

    # Country presets — ENGLISH names (Metrika API returns countries in English!)
    EU_COUNTRIES = [
    ...
    # Split into two halves so each fits Metrika's ~20-condition filter limit
    EU1_COUNTRIES = EU_COUNTRIES[:14]   # Austria … Ireland
    EU2_COUNTRIES = EU_COUNTRIES[14:]   # Italy … Sweden
    EN_COUNTRIES = ["United States", "United Kingdom", "Australia",
                    "New Zealand", "Canada", "Ireland"]

    # Get available country names (English, as Metrika returns them)
    country_raw = stat("ym:s:visits", dims="ym:s:regionCountryName", limit=100)
    all_countries = []
    if "data" in country_raw:
        for row in country_raw["data"]:
            n = row["dimensions"][0].get("name", "")
            if n: all_countries.append(n)

    # Preset selector
    preset = st.selectbox("🌍 Страны — пресет", [
        "Все страны",
        "🇪🇺 Европа 1 (А–И)",
        "🇪🇺 Европа 2 (И–Ш)",
        "🇬🇧 Англоязычные",
    ...
    if preset == "Все страны":
        selected_countries = []
    elif preset == "🇪🇺 Европа 1 (А–И)":
        selected_countries = list(EU1_COUNTRIES)
    elif preset == "🇪🇺 Европа 2 (И–Ш)":
        selected_countries = list(EU2_COUNTRIES)
    elif preset == "🇬🇧 Англоязычные":
        selected_countries = list(EN_COUNTRIES)
    else:  # manual
        opts = all_countries if all_countries else EU_COUNTRIES
        selected_countries = st.multiselect("Выберите страны", opts, default=[])

    # Cap at 18 to stay under Metrika's 20-condition limit (robot + blog take 2)
    COUNTRY_CAP = 18
    if len(selected_countries) > COUNTRY_CAP:
        st.warning(f"⚠️ Выбрано {len(selected_countries)} стран, лимит ~{COUNTRY_CAP}. "
                   f"Беру {COUNTRY_CAP} крупнейших по трафику.")
        selected_countries = [c for c in all_countries
                              if c in selected_countries][:COUNTRY_CAP]

    # Show the country list for EU halves so user sees what's included
    if preset.startswith("🇪🇺"):
        st.caption(f"Включено: {', '.join(selected_countries)}")

    country = preset  # for display/badge

    # Blog filter
    blog_mode = st.radio("📝 Блог (/blog)", ["Весь трафик", "Только с /blog", "Без /blog"],
                         index=0)
    ...
    device_label = st.selectbox("💻 Устройство", list(DEVICE_MAP.keys()))
    device_val = DEVICE_MAP[device_label]
    ...
    # Landing page — dropdown of top pages OR manual entry
    land_raw = stat("ym:s:visits", dims="ym:s:startURL", limit=30)
    HOME_EXACT = "🏠 Только главная (точно /)"
    MANUAL_ENTRY = "✏️ Ввести путь вручную"
    land_options = {"Все страницы входа": None, HOME_EXACT: "__HOME__"}
    land_labels = ["Все страницы входа", HOME_EXACT, MANUAL_ENTRY]
    if "data" in land_raw:
        for row in land_raw["data"]:
            ...
            if full_url:
                ...
                short = path[:60]
                if short not in land_options:
                    land_labels.append(short)
                    land_options[short] = path
    landing_label = st.selectbox("🚪 Страница входа", land_labels)

    manual_path = None
    manual_mode = "contains"
    if landing_label == MANUAL_ENTRY:
        manual_path = st.text_input("Введите путь", placeholder="/collections/psychedelic",
                                    help="Например: /collections/psychedelic или /cannabis-seeds/blue-dream")
        manual_mode = st.radio("Как искать", ["Точное совпадение", "Содержит"],
                               horizontal=True, index=0)
        landing_path = "__MANUAL__" if manual_path else None
    else:
    ...
    if selected_countries:
        if len(selected_countries) == 1:
            filters.append(f"ym:s:regionCountryName=='{q(selected_countries[0])}'")
        else:
            cc = " OR ".join(
                f"ym:s:regionCountryName=='{q(c)}'" for c in selected_countries
            )
            filters.append(f"({cc})")
    if traffic_src_id:
        filters.append(f"ym:s:trafficSource=='{q(traffic_src_id)}'")
    if landing_path == "__HOME__":
        # Exact home page — only herbiesheadshop.com/ (no /au/, /uk/, no slugs)
        filters.append("ym:s:startURL=='https://herbiesheadshop.com/'")
    elif landing_path == "__MANUAL__" and manual_path:
        mp = manual_path.strip()
        if manual_mode == "Точное совпадение":
            # Build full URL for exact match
            full = f"https://herbiesheadshop.com{mp}" if mp.startswith("/") else mp
            filters.append(f"ym:s:startURL=='{q(full)}'")
        else:
            filters.append(f"ym:s:startURL=@'{q(mp)}'")
    elif landing_path:
        filters.append(f"ym:s:startURL=@'{q(landing_path)}'")
    # Blog filter
    if blog_mode == "Только с /blog":
        filters.append("ym:s:startURL=@'/blog'")
    elif blog_mode == "Без /blog":
        filters.append("ym:s:startURL!@'/blog'")
    # Device filter
    if device_val:
        filters.append(f"ym:s:deviceCategory=='{device_val}'")
    c_flt = " AND ".join(filters) if filters else None

    # Active filter badge
    active = []
    if selected_countries:
        if preset == "✏️ Выбрать вручную":
            active.append(f"{len(selected_countries)} стран")
        else:
            active.append(preset)
    if traffic_src_id: active.append(traffic_src)
    if landing_path == "__MANUAL__" and manual_path:
        active.append(f"стр: {manual_path}")
    elif landing_path:
        active.append(landing_label)
    if blog_mode != "Весь трафик": active.append(blog_mode)
    if device_val: active.append(device_label)
    if active:
...
FUNNEL_DEF = [
    ...
    ("Нажал Create order","565668425",         "create_order_click", "нажал create"),
...
with st.spinner("Загрузка воронки..."):
    ...
    with st.expander("🎯 Цели воронки", expanded=False):
...
# ══════════════════════════════════════════════════════════════════════════════
# БЛОК 3c — Посессионные пары по дням (составные цели)
# ══════════════════════════════════════════════════════════════════════════════
st.markdown('<div class="sec">◈ посессионная конверсия по парам — оба шага в одной сессии</div>',
            unsafe_allow_html=True)
st.caption("Данные из составных целей Метрики. Значения всегда ≤100% — считаются только сессии где оба шага произошли последовательно.")

with st.spinner("Загрузка посессионных данных по парам..."):
    pair_daily_data = {}
    pair_denom_daily = {}
    for pair_name, pair_gid, denom_kw in PAIR_GOALS:
        # Daily numerator: composite goal
        d_num = stat(f"{base_m},ym:s:goal{pair_gid}{goal_sfx}",
                     dims="ym:s:date", date1=d1_main, date2=d2_main,
                     flt=c_flt, limit=90)
        if "data" in d_num:
            pair_daily_data[pair_name] = {
                row["dimensions"][0]["name"]: row["metrics"][1]
                for row in d_num["data"]
            }
        # Daily denominator: first stage goal
        denom_gid = find_goal(goals, denom_kw)
        if denom_gid:
            d_den = stat(f"{base_m},ym:s:goal{denom_gid}{goal_sfx}",
                         dims="ym:s:date", date1=d1_main, date2=d2_main,
                         flt=c_flt, limit=90)
            if "data" in d_den:
                pair_denom_daily[pair_name] = {
                    row["dimensions"][0]["name"]: row["metrics"][1]
                    for row in d_den["data"]
                }

# Draw 2x2 grid of charts
pair_list = list(PAIR_GOALS)
for row_idx in range(0, len(pair_list), 2):
    cols_pair = st.columns(2)
    for col_idx, col in enumerate(cols_pair):
        idx = row_idx + col_idx
        if idx >= len(pair_list):
            break
        pair_name, pair_gid, denom_kw = pair_list[idx]
        num_by_day  = pair_daily_data.get(pair_name, {})
        den_by_day  = pair_denom_daily.get(pair_name, {})
        all_days_p  = sorted(set(num_by_day) | set(den_by_day))
        x_labels_p  = [d[8:10] + "." + d[5:7] for d in all_days_p]
        conv_p = []
        for day in all_days_p:
            n = num_by_day.get(day, 0)
            d = den_by_day.get(day, 0)
            conv_p.append(round(n / d * 100, 2) if d else 0)

        avg_p = round(sum(conv_p) / len(conv_p), 1) if conv_p else 0
        total_n = sum(num_by_day.values())
        total_d = sum(den_by_day.values())
        total_pct = round(total_n / total_d * 100, 1) if total_d else 0

        with col:
            st.markdown(f"""<div style='font-family:IBM Plex Mono,monospace;
            font-size:.7rem;color:#888;margin-bottom:.3rem'>
            <span style='color:#00e676'>●</span> {pair_name}
            &nbsp;&nbsp;
            <span style='color:#00e676;font-size:.9rem;font-weight:600'>{total_pct}%</span>
            <span style='color:#444'> за период &nbsp;·&nbsp; {fmt(total_n)} из {fmt(total_d)}</span>
            </div>""", unsafe_allow_html=True)

            if all_days_p and any(v > 0 for v in conv_p):
                fig_p = go.Figure()
                fig_p.add_trace(go.Scatter(
                    x=x_labels_p, y=conv_p,
                    mode="lines+markers",
                    line=dict(color="#00e676", width=2),
                ...
                fig_p.add_hline(y=avg_p, line_dash="dot", line_color="#333",
                                annotation_text=f"ср. {avg_p}%",
                                annotation_font=dict(color="#444", size=9))
                fig_p.update_layout(
                    paper_bgcolor="#0d0d0d", plot_bgcolor="#161616",
                    font=dict(family="IBM Plex Mono", color="#888", size=9),
                    margin=dict(l=5, r=10, t=5, b=5), height=180,
                    xaxis=dict(type="category", gridcolor="#1e1e1e",
                               zeroline=False,
                               tickangle=-45 if len(all_days_p) > 14 else 0),
                ...
                font-size:.65rem;color:#333;padding:.5rem;background:#111;
                border-radius:4px;text-align:center'>
                Данные накапливаются — составная цель работает с момента создания
                </div>""", unsafe_allow_html=True)
...
with st.spinner(""):
    # Find purchase goal id for per-source conversion
    purchase_gid = find_goal(goals, "purchase", "покупка", "ecommerce: покупка")
    purchase_metric = f",ym:s:goal{purchase_gid}visits" if purchase_gid else ""

    src_m = stat(f"ym:s:visits{purchase_metric}", dims="ym:s:trafficSource",
                 date1=d1_main, date2=d2_main, flt=c_flt, limit=20)
    src_c = stat(f"ym:s:visits{purchase_metric}", dims="ym:s:trafficSource",
                 date1=d1_cmp,  date2=d2_cmp,  flt=c_flt, limit=20)

if "data" in src_m and src_m["data"]:
    def to_df(d, with_purchases=False):
        rows = []
        for r in d.get("data", []):
            row = {"src": r["dimensions"][0].get("name","?"),
                   "v": int(r["metrics"][0])}
            if with_purchases:
                row["p"] = int(r["metrics"][1]) if len(r["metrics"]) > 1 else 0
            rows.append(row)
        return pd.DataFrame(rows)
    df_m = to_df(src_m, with_purchases=bool(purchase_gid))
    df_c = to_df(src_c, with_purchases=bool(purchase_gid))
    ...
    with col2:
        if purchase_gid and "p_A" in top.columns:
            def cr(p, v): return f"{p/v*100:.2f}%" if v else "0%"
            df_show = pd.DataFrame({
                "Источник": top["src"],
                f"Сеансы {label_main}": top["v_A"].astype(int),
                f"Сеансы {label_cmp}": top["v_B"].astype(int),
                f"Покупки {label_main}": top["p_A"].astype(int),
                f"Покупки {label_cmp}": top["p_B"].astype(int),
                f"CR {label_main}": [cr(p, v) for p, v in zip(top["p_A"], top["v_A"])],
                f"CR {label_cmp}": [cr(p, v) for p, v in zip(top["p_B"], top["v_B"])],
                "Δ покупок": (top["p_A"] - top["p_B"]).astype(int),
            })
