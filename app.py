...
with st.sidebar:
    ...
    # Dynamically resolve country name → region ID from API (reliable, no guessing)
    def get_eu_region_ids():
        d = stat("ym:s:visits", dims="ym:s:regionCountry",
                 date1="30daysAgo", date2="today", limit=300)
        ids = []
        debug_rows = []
        if "data" in d:
            for row in d["data"]:
                ...
                if cname in EU_COUNTRY_NAMES and cid is not None:
                    ids.append(str(cid))
        return ids, debug_rows
    EU_REGION_IDS_LIST, _eu_debug_rows = get_eu_region_ids()
    ...
    if country == "🇪🇺 Европа (ЕС)":
        # OR-join all EU countries by region ID (resolved dynamically from API)
