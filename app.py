...
with st.sidebar:
    ...
    if selected_countries:
        ...
        if sel_ids and len(sel_ids) == len(selected_countries):
            # All IDs resolved — use IN() with numeric IDs (1 condition, reliable)
            id_list = ",".join(f"'{i}'" for i in sel_ids)
            filters.append(f"ym:s:regionCountry IN({id_list})")
        else:
            # Some/all IDs missing — OR by name (works for strings, unlike IN by name)
            cc = " OR ".join(
