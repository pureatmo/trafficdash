...
with st.sidebar:
    ...
    if country == "🇪🇺 Европа (ЕС)":
        if EU_REGION_IDS_LIST:
            # Preferred: filter by numeric region ID
            eu_filter = " OR ".join(
                f"ym:s:regionCountry=='{i}'" for i in EU_REGION_IDS_LIST
            )
            filters.append(f"({eu_filter})")
        else:
            # Fallback: filter by country NAME (русские имена из API)
            eu_filter = " OR ".join(
                f"ym:s:regionCountryName=='{q(c)}'" for c in EU_COUNTRY_NAMES
            )
            filters.append(f"({eu_filter})")
