...
with st.sidebar:
    ...
    def get_eu_region_ids():
        d = stat("ym:s:visits", dims="ym:s:regionCountry,ym:s:regionCountryName",
                 date1="2025-01-01", date2="today", limit=300)
        ids = []
        if "data" in d:
            for row in d["data"]:
                cid  = row["dimensions"][0].get("id") or row["dimensions"][0].get("name")
                cname = row["dimensions"][1].get("name", "")
                if cname in EU_COUNTRY_NAMES and cid:
                    ids.append(str(cid))
        return ids
    EU_REGION_IDS_LIST = get_eu_region_ids()
...
with st.expander("🔧 Отладка фильтров", expanded=False):
    ...
    if country == "🇪🇺 Европа (ЕС)":
        ...
        for c in sorted(EU_COUNTRY_NAMES):
            mark = "🟢" if c in found_names else "⚪"
            st.write(f"{mark} {c}")
