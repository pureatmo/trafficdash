...
with st.sidebar:
    ...
    # Build list of available countries from API
    ...
    if preset == "Все страны":
        ...
        selected_countries = [c for c in EU1_COUNTRIES if c in all_countries]
    elif preset == "🇪🇺 Европа 2 (И–Ш)":
        selected_countries = [c for c in EU2_COUNTRIES if c in all_countries]
    elif preset == "🇬🇧 Англоязычные":
        selected_countries = [c for c in EN_COUNTRIES if c in all_countries]
    else:  # manual
        selected_countries = st.multiselect("Выберите страны", all_countries,
                                             default=[])
    ...
    if selected_countries:
        if len(selected_countries) == 1:
            ...
            # OR-join (each country = 1 condition); capped above to stay under limit
            cc = " OR ".join(
